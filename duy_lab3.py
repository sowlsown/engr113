...
async def play(robot):
    ...
    startingpos = await getpos(robot)
    n_s = 0
    ...
    while True:
        ...
        # searches for gap
            ...
            if sensors[6] < th:
                ...
                openingpos1 = await getpos(robot)
                speed = 30
                n_s = 2
            elif sensors[3] > th:
                speed = -30
        ...
        # rerun
            ...
            await robot.navigate_to(openingpos1.y)
            await robot.navigate_to(openingpos2.x)
            await robot.navigate_to(openingpos2.y)
            await robot.navigate_to(endingpos.x)
            n_s = 6
