...
def conv_to_cm(sensor_value):
    if sensor_value <= 0:
        return 25
    return m.log(sensor_value / 5296) / (-.348)
...
async def play(robot):
    ...
    while True:
        ...
        if n_s == 0:
            ...
            error = distance - set_point #calculates how far it is from the wall
            correction = Kp * error #the correction value times the distance
            
            left_speed = (1 - correction) * speed
            right_speed = (1 + correction) * speed
            ...
            if sensors[FRONT] > th and sensors[RIGHT] > th:
                ...
                await robot.set_wheel_speeds(left_speed, right_speed) #this allows it to correct itself.
